...
from dataset_experiments import create_data_loaderData_PAIR
import torch
...
for count in range(1, 11):
    ...
    # no shuffle
    validation_loader, validation_length = create_data_loaderData_PAIR(validation_data, tokenizer, MAX_LEN_BERT,
                                                                      BATCH_SIZE, eval=True)

    test_loader, test_length = create_data_loaderData_PAIR(test_data, tokenizer, MAX_LEN_BERT, BATCH_SIZE, eval=True)
    ...
    model = torch.load('PAIR'+str(count) + '.pth')  # save best model
    model.to(device)
    ...
    with open("report_PAIR_val" + str(seed) + ".txt", 'w') as f:
    ...
    with open("report_PAIR_test" + str(seed) + ".txt", 'w') as f:
        f.write("TEST TIME\n")
        f.write(report)

    # save prediction in a file txt
    with open('test_pred_PAIR' + str(seed) + '.txt', 'w') as f:
        for item in y_pred:
            f.write("%s\n" % item)
