...
from dataset_experiments import create_data_loaderData_SINGLE
import torch
...
for count in range(1, 11):
    ...
    # no shuffle
    validation_loader, validation_length = create_data_loaderData_SINGLE(validation_data, tokenizer, MAX_LEN_BERT,
                                                                      BATCH_SIZE, eval=True)

    test_loader, test_length = create_data_loaderData_SINGLE(test_data, tokenizer, MAX_LEN_BERT, BATCH_SIZE, eval=True)
    ...
    model = torch.load('SINGLE' + str(count) + '.pth')  # save best model
    ...
    with open("report_SINGLE_val" + str(seed) + ".txt", 'w') as f:
    ...
    with open("report_SINGLE_test" + str(seed) + ".txt", 'w') as f:
        f.write("TEST TIME\n")
        f.write(report)

    # save prediction in a file txt
    with open('test_pred_SINGLE' + str(seed) + '.txt', 'w') as f:
        for item in y_pred:
            f.write("%s\n" % item)
