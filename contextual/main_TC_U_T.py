...
for count in range(1, 11):
    ...
    train_loader, train_length = create_data_loaderData_TC_U_T(train_data, tokenizer, MAX_LEN_BERT, BATCH_SIZE)
    # no shuffle
    ...
    while go:
        ...
        if val_loss < best_loss:
            torch.save(model, 'TC_U_T' + str(count) + '.pth')  # save best model
            best_loss = val_loss
            no_increase = 0
        else:
            no_increase += 1
