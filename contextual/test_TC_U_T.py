...
from dataset_experiments import create_data_loaderData_TC_U_T
import torch
...
for count in range(1, 11):
    ...
    tokenizer.add_tokens(['<t>', '</t>', '<o>', '</o>'], special_tokens=True)

    validation_loader, validation_length = create_data_loaderData_TC_U_T(validation_data, tokenizer, MAX_LEN_BERT,
                                                                            BATCH_SIZE, eval=True)

    test_loader, test_length = create_data_loaderData_TC_U_T(test_data, tokenizer, MAX_LEN_BERT, BATCH_SIZE, eval=True)
    ...
    model = torch.load('TC_U_T' + str(count) + '.pth')  # save best model
    ...
    with open("report_TC_U_T_val" + str(seed) + ".txt", 'w') as f:
    ...
    with open("report_TC_U_T_test" + str(seed) + ".txt", 'w') as f:
        f.write("TEST TIME\n")
        f.write(report)

    # save prediction in a file txt
    with open('test_pred_TC_U_T' + str(seed) + '.txt', 'w') as f:
        for item in y_pred:
            f.write("%s\n" % item)
