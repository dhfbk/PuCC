...
for count in range(1, 11):
    ...
    MAX_LEN_BERT = 512
    BATCH_SIZE = 32 #it is 32 in the original

    # create dataloaders for training and validation sets
    print("Creating Training set...")
    train_loader, train_length = create_data_loaderData_TC(train_data, tokenizer, MAX_LEN_BERT, BATCH_SIZE)
    # no shuffle
    ...
    while go:
        ...
        if val_loss < best_loss:
            torch.save(model, 'TC'+str(count) + '.pth')  # save best model
            best_loss = val_loss
            no_increase = 0
        else:
            no_increase += 1
